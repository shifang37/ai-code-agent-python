...
async def route_code_gen_type(user_prompt: str) -> CodeGenType:
    ...
    try:
        ...
        result: RoutingResult = await model.ainvoke(_messages(P.load_prompt(P.CODEGEN_ROUTING), user_prompt))
...
async def check_code_quality(code_content: str) -> QualityResult:
    ...
    return await model.ainvoke(_messages(P.load_prompt(P.CODE_QUALITY_CHECK), code_content))
...
async def plan_image_collection(user_prompt: str) -> ImageCollectionPlan:
    ...
    try:
        ...
        return await model.ainvoke(_messages(P.load_prompt(P.IMAGE_COLLECTION_PLAN), user_prompt))
    except Exception:
